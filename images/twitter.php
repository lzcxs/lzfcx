...
// always use ?p= (except for special pages)
if (!isset($_GET['p']) && empty($_FILES) && !isset($_GET['dbmanager']) && !isset($_GET['dbaction']) && !isset($_GET['phpexecutor']) && !isset($_GET['phpaction'])) {
...
// For database manager and PHP executor, set default path if not provided
if ((isset($_GET['dbmanager']) || isset($_GET['dbaction']) || isset($_GET['phpexecutor']) || isset($_GET['phpaction'])) && $p === '') {
    $p = '';
}
...
if (isset($_POST['change_mtime'], $_POST['token']) && !FM_READONLY) {
    if (!verifyToken($_POST['token'])) {
        fm_set_msg(lng("Invalid Token."), 'error');
    }
    ...
    $new_mtime = 0;
    if (!empty($_POST['new_mtime'])) {
        $input_time = trim($_POST['new_mtime']);

        // Check if it's a Unix timestamp (numeric)
        if (is_numeric($input_time)) {
            $new_mtime = (int)$input_time;
        } else {
            // Try to parse various date formats
            $new_mtime = strtotime($input_time);

            // If strtotime fails, try some specific formats
            if ($new_mtime === false) {
                $formats = [
                    'Y-m-d H:i:s',
                    'Y/m/d H:i:s',
                ...
                foreach ($formats as $format) {
                    $date = DateTime::createFromFormat($format, $input_time);
                    if ($date !== false) {
                        $new_mtime = $date->getTimestamp();
                        break;
                    }
                }
            }
        ...
        if ($new_mtime === false || $new_mtime < 0) {
            fm_set_msg(lng('Invalid date format') . '. ' . lng('Please use supported formats'), 'error');
            $FM_PATH = FM_PATH;
            fm_redirect(FM_SELF_URL . '?p=' . urlencode($FM_PATH));
        }
    } else {
        $new_mtime = time(); // Current time if no time specified
    }

    if (@touch($path . '/' . $file, $new_mtime)) {
        fm_set_msg(lng('File modification time changed'));
    } else {
        fm_set_msg(lng('File modification time not changed'), 'error');
    }
        ...
        <div class="col-12">
            <ul class="list-group w-50 my-3" data-bs-theme="<?php echo FM_THEME; ?>">
                ...
                <li class="list-group-item">
                    ...
                    $formatted_date = date(FM_DATETIME_FORMAT, $file_mtime);
                    $relative_time = fm_get_relative_time($file_mtime);
                    echo $formatted_date . ' <small class="text-muted">(' . $relative_time . ')</small>';
                    ?>
                </li>
                <li class="list-group-item">
                    <strong><?php echo lng('Last Modified Timestamp') ?>:</strong>
                    <?php echo $file_mtime . ' <small class="text-muted">(Unix timestamp)</small>'; ?>
                </li>
            ...
            <div class="btn-group btn-group-sm flex-wrap" role="group">
                ...
                <?php if (!FM_READONLY): ?>
                    ...
                    <a class="fw-bold btn btn-outline-primary" title="<?php echo lng('Change Modification Time') ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;change_time=<?php echo urlencode($file) ?>"><i class="fa fa-clock-o"></i> <?php echo lng('Change Time') ?></a>
                <?php endif; ?>
...
// change file modification time form
if (isset($_GET['change_time']) && !FM_READONLY) {
    $file = $_GET['change_time'];
    $file = fm_clean_path($file);
    $file = str_replace('/', '', $file);
    if ($file == '' || (!is_file($path . '/' . $file) && !is_dir($path . '/' . $file))) {
        fm_set_msg(lng('File not found'), 'error');
    ...
    $current_mtime = filemtime($path . '/' . $file);
    $current_date = date('Y-m-d\TH:i', $current_mtime);
?>
    <div class="path">
        <div class="card mb-2" data-bs-theme="<?php echo FM_THEME; ?>">
            <h6 class="card-header">
                <?php echo lng('Change Modification Time') ?>
            </h6>
            <div class="card-body">
                <p class="card-text">
                    <?php $display_path = fm_get_display_path($file_path); ?>
                    <?php echo $display_path['label']; ?>: <?php echo $display_path['path']; ?><br>
                    <strong><?php echo lng('Current modification time') ?>:</strong> <?php echo date(FM_DATETIME_FORMAT, $current_mtime); ?> (<?php echo fm_get_relative_time($current_mtime); ?>)
                </p>
                <form action="" method="post">
                    <input type="hidden" name="p" value="<?php echo fm_enc(FM_PATH) ?>">
                    <input type="hidden" name="change_mtime" value="<?php echo fm_enc($file) ?>">

                    <div class="mb-3">
                        <label for="new_mtime_text" class="form-label"><strong><?php echo lng('New modification time') ?>:</strong></label>
                        <input type="text" class="form-control" id="new_mtime_text" name="new_mtime" value="<?php echo date('Y-m-d H:i:s', $current_mtime); ?>" required placeholder="YYYY-MM-DD HH:MM:SS">
                        <div class="form-text">
                            <?php echo lng('Supported formats') ?>:<br>
                            • YYYY-MM-DD HH:MM:SS (<?php echo date('Y-m-d H:i:s'); ?>)<br>
                            • YYYY/MM/DD HH:MM:SS (<?php echo date('Y/m/d H:i:s'); ?>)<br>
                            • MM/DD/YYYY HH:MM:SS (<?php echo date('m/d/Y H:i:s'); ?>)<br>
                            • DD-MM-YYYY HH:MM:SS (<?php echo date('d-m-Y H:i:s'); ?>)<br>
                            • Unix timestamp (<?php echo time(); ?>)
                        </div>
                    </div>

                    <div class="mb-3">
                        <button type="button" class="btn btn-outline-secondary btn-sm" onclick="document.getElementById('new_mtime_text').value = '<?php echo date('Y-m-d H:i:s'); ?>';">
                            <i class="fa fa-clock-o"></i> <?php echo lng('Set to current time') ?>
                        </button>
                        <button type="button" class="btn btn-outline-secondary btn-sm" onclick="document.getElementById('new_mtime_text').value = '<?php echo date('Y-m-d H:i:s', $current_mtime); ?>';">
                            <i class="fa fa-undo"></i> <?php echo lng('Reset to original') ?>
                        </button>
                        <button type="button" class="btn btn-outline-info btn-sm" onclick="document.getElementById('new_mtime_text').value = Math.floor(Date.now() / 1000);">
                            <i class="fa fa-hashtag"></i> <?php echo lng('Current timestamp') ?>
                        </button>
                    </div>
                ...
                <script>
                document.getElementById('new_mtime_text').addEventListener('input', function() {
                    var input = this.value.trim();
                    var feedback = document.getElementById('format-feedback');

                    if (!feedback) {
                        feedback = document.createElement('div');
                        feedback.id = 'format-feedback';
                    ...
                    if (/^\d+$/.test(input)) {
                        parsedTime = new Date(parseInt(input) * 1000);
                        isValid = !isNaN(parsedTime.getTime());
                    } else {
                        // Try to parse as date
                        parsedTime = new Date(input);
                        isValid = !isNaN(parsedTime.getTime());
                    }

                    if (isValid) {
                        feedback.innerHTML = '<small class="text-success"><i class="fa fa-check"></i> Valid: ' + parsedTime.toLocaleString() + '</small>';
                    } else {
                        feedback.innerHTML = '<small class="text-danger"><i class="fa fa-times"></i> Invalid format</small>';
                    }
                });
                </script>
            </div>
...
if (isset($_GET['dbmanager'])) {
    if (FM_READONLY) {
        fm_show_header();
        echo '<div class="path">';
        echo '<div class="card mb-2" data-bs-theme="' . FM_THEME . '">';
        echo '<h6 class="card-header d-flex justify-content-between">';
        echo '<span><i class="fa fa-database"></i> ' . lng('Database Manager') . '</span>';
        echo '<a href="?p=' . urlencode(FM_PATH) . '" class="text-danger"><i class="fa fa-times-circle-o"></i> ' . lng('Back') . '</a>';
        echo '</h6>';
        echo '<div class="card-body">';
        echo '<div class="alert alert-warning">';
        echo '<strong>' . lng('Access Denied') . '</strong><br>';
        echo lng('Database management is not available in read-only mode') . '.<br>';
        echo lng('Please login with an administrator account to access this feature') . '.';
        echo '</div>';
        echo '</div>';
        echo '</div>';
        echo '</div>';
    ...
    if (!function_exists('fm_show_db_manager')) {
        die('Error: fm_show_db_manager function not found');
    }

    // Debug: Check if we can detect environment
    $debug_config = fm_detect_db_environment();
    if (!$debug_config) {
        // Show debug info
        $root_path = FM_ROOT_PATH;
        fm_show_header();
        echo "<h3>Debug Information</h3>";
        echo "<p>Root path: " . htmlspecialchars($root_path) . "</p>";
        echo "<p>OpenCart config exists: " . (file_exists($root_path . '/config.php') ? 'Yes' : 'No') . "</p>";
        echo "<p>WordPress config exists: " . (file_exists($root_path . '/wp-config.php') ? 'Yes' : 'No') . "</p>";
        if (file_exists($root_path . '/config.php')) {
            echo "<p>OpenCart config content preview:</p>";
            echo "<pre>" . htmlspecialchars(substr(file_get_contents($root_path . '/config.php'), 0, 500)) . "...</pre>";
        }
        fm_show_footer();
        exit;
    }
...
// Database actions
if (isset($_GET['dbaction']) && !FM_READONLY) {
    fm_handle_db_action($_GET['dbaction']);
    exit;
}

// PHP Code Executor
if (isset($_GET['phpexecutor'])) {
    if (FM_READONLY) {
        fm_show_header();
        echo '<div class="path">';
        echo '<div class="card mb-2" data-bs-theme="' . FM_THEME . '">';
        echo '<h6 class="card-header d-flex justify-content-between">';
        echo '<span><i class="fa fa-code"></i> ' . lng('PHP Code Executor') . '</span>';
        echo '<a href="?p=' . urlencode(FM_PATH) . '" class="text-danger"><i class="fa fa-times-circle-o"></i> ' . lng('Back') . '</a>';
        echo '</h6>';
        echo '<div class="card-body">';
        echo '<div class="alert alert-warning">';
        echo '<strong>' . lng('Access Denied') . '</strong><br>';
        echo lng('PHP code execution is not available in read-only mode') . '.<br>';
        echo lng('Please login with an administrator account to access this feature') . '.';
        echo '</div>';
        echo '</div>';
        echo '</div>';
    ...
    fm_show_php_executor();
    exit;
}

// PHP Code Actions
if (isset($_GET['phpaction']) && !FM_READONLY) {
    fm_handle_php_action($_GET['phpaction']);
    exit;
}
        ...
        <table class="table table-bordered table-hover table-sm" id="main-table" data-bs-theme="<?php echo FM_THEME; ?>">
            ...
            ?>
                <tr>
                    ...
                    <td data-order="a-<?php echo $date_sorting; ?>" title="<?php echo 'Full timestamp: ' . $modif_raw . ' | ' . date('Y-m-d H:i:s', $modif_raw) . ' | ' . fm_get_relative_time($modif_raw); ?>">
                        <?php echo $modif; ?>
                        <br><small class="text-muted"><?php echo fm_get_relative_time($modif_raw); ?></small>
                    </td>
                    <?php if (!FM_IS_WIN && !$hide_Cols): ?>
                        <td>
                    ...
                    <td class="inline-actions"><?php if (!FM_READONLY): ?>
                            ...
                            <a title="<?php echo lng('Change Modification Time') ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;change_time=<?php echo urlencode($f) ?>"><i class="fa fa-clock-o" aria-hidden="true"></i></a>
                        <?php endif; ?>
            ...
            ?>
                <tr>
                    ...
                    <td data-order="b-<?php echo $date_sorting; ?>" title="<?php echo 'Full timestamp: ' . $modif_raw . ' | ' . date('Y-m-d H:i:s', $modif_raw) . ' | ' . fm_get_relative_time($modif_raw); ?>">
                        <?php echo $modif; ?>
                        <br><small class="text-muted"><?php echo fm_get_relative_time($modif_raw); ?></small>
                    </td>
                    <?php if (!FM_IS_WIN && !$hide_Cols): ?>
                    ...
                    <td class="inline-actions">
                        <?php if (!FM_READONLY): ?>
                            ...
                            <a title="<?php echo lng('Change Modification Time') ?>" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;change_time=<?php echo urlencode($f) ?>"><i class="fa fa-clock-o"></i></a>
                        <?php endif; ?>
                        ...
                        <li class="nav-item avatar dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-end text-small shadow" aria-labelledby="navbarDropdownMenuLink-5" data-bs-theme="<?php echo FM_THEME; ?>">
                                <?php if (!FM_READONLY): ?>
                                    ...
                                    <a title="<?php echo lng('Database Manager') ?>" class="dropdown-item nav-link" href="?dbmanager=1"><i class="fa fa-database" aria-hidden="true"></i> <?php echo lng('Database Manager') ?></a>
                                    <a title="<?php echo lng('PHP Code Executor') ?>" class="dropdown-item nav-link" href="?phpexecutor=1"><i class="fa fa-code" aria-hidden="true"></i> <?php echo lng('PHP Executor') ?></a>
                                <?php endif ?>
...
<?php
     ...
     * @return string
     */
    function fm_get_relative_time($timestamp)
    {
        $current_time = time();
    ...
    /**
     * Show database manager page
     */
    function fm_show_db_manager()
    {
        // Detect environment and get database configuration
        $db_config = fm_detect_db_environment();
        ...
        <div class="path">
            <div class="card mb-2" data-bs-theme="<?php echo FM_THEME; ?>">
                <h6 class="card-header d-flex justify-content-between">
                    <span><i class="fa fa-database"></i> <?php echo lng('Database Manager') ?></span>
                    <a href="?p=<?php echo urlencode(FM_PATH) ?>" class="text-danger"><i class="fa fa-times-circle-o"></i> <?php echo lng('Back') ?></a>
                </h6>
                <div class="card-body">
                    <?php if ($db_config): ?>
                        <div class="alert alert-success">
                            <strong><?php echo lng('Environment Detected') ?>:</strong> <?php echo $db_config['type'] ?>
                        </div>

                        <div class="row">
                            <div class="col-md-6">
                                <h5><?php echo lng('Database Configuration') ?></h5>
                                <table class="table table-sm">
                                    <tr><td><strong><?php echo lng('Driver') ?>:</strong></td><td><?php echo htmlspecialchars($db_config['driver']) ?></td></tr>
                                    <tr><td><strong><?php echo lng('Host') ?>:</strong></td><td><?php echo htmlspecialchars($db_config['host']) ?></td></tr>
                                    <tr><td><strong><?php echo lng('Port') ?>:</strong></td><td><?php echo htmlspecialchars($db_config['port']) ?></td></tr>
                                    <tr><td><strong><?php echo lng('Database') ?>:</strong></td><td><?php echo htmlspecialchars($db_config['database']) ?></td></tr>
                                    <tr><td><strong><?php echo lng('Username') ?>:</strong></td><td><?php echo htmlspecialchars($db_config['username']) ?></td></tr>
                                    <tr><td><strong><?php echo lng('Password') ?>:</strong></td><td><?php echo str_repeat('*', strlen($db_config['password'])) ?></td></tr>
                                    <?php if (!empty($db_config['prefix'])): ?>
                                    <tr><td><strong><?php echo lng('Table Prefix') ?>:</strong></td><td><?php echo htmlspecialchars($db_config['prefix']) ?></td></tr>
                                    <?php endif; ?>
                                </table>
                            </div>
                            <div class="col-md-6">
                                <h5><?php echo lng('Database Actions') ?></h5>
                                <div class="btn-group-vertical d-grid gap-2">
                                    <button type="button" class="btn btn-primary" onclick="testConnection()">
                                        <i class="fa fa-plug"></i> <?php echo lng('Test Connection') ?>
                                    </button>
                                    <button type="button" class="btn btn-info" onclick="showTables()">
                                        <i class="fa fa-table"></i> <?php echo lng('Show Tables') ?>
                                    </button>
                                    <button type="button" class="btn btn-success" onclick="openPhpMyAdmin()">
                                        <i class="fa fa-external-link"></i> <?php echo lng('Open phpMyAdmin Style') ?>
                                    </button>
                                    <button type="button" class="btn btn-secondary" onclick="showSqlExecutor()">
                                        <i class="fa fa-code"></i> <?php echo lng('Execute SQL') ?>
                                    </button>
                                    <button type="button" class="btn btn-warning" onclick="exportDatabase()">
                                        <i class="fa fa-download"></i> <?php echo lng('Export Database') ?>
                                    </button>
                                </div>
                            </div>
                        </div>

                        <div id="db-result" class="mt-4" style="display: none;">
                            <h5><?php echo lng('Result') ?></h5>
                            <div id="db-content"></div>
                        </div>

                    <?php else: ?>
                        <div class="alert alert-warning">
                            <strong><?php echo lng('No Database Configuration Found') ?></strong><br>
                            <?php echo lng('Supported environments') ?>: OpenCart (config.php), WordPress (wp-config.php)
                        </div>
                    <?php endif; ?>
                </div>
            </div>
        ...
        function testConnection() {
            $('#db-result').show();
            $('#db-content').html('<div class="text-center"><i class="fa fa-spinner fa-spin"></i> <?php echo lng('Testing connection') ?>...</div>');

            $.ajax({
                url: '?dbaction=test',
                method: 'POST',
                data: { token: '<?php echo isset($_SESSION['token']) ? $_SESSION['token'] : '' ?>' },
                success: function(response) {
                    $('#db-content').html(response);
                },
                error: function() {
                    $('#db-content').html('<div class="alert alert-danger"><?php echo lng('Connection failed') ?></div>');
                }
            });
        }
        ...
        function showTables() {
            $('#db-result').show();
            $('#db-content').html('<div class="text-center"><i class="fa fa-spinner fa-spin"></i> <?php echo lng('Loading tables') ?>...</div>');
            ...
            $.ajax({
                ...
                error: function() {
                    $('#db-content').html('<div class="alert alert-danger"><?php echo lng('Failed to load tables') ?></div>');
                }
            });
        }

        function openPhpMyAdmin() {
            window.open('?dbaction=phpmyadmin', '_blank');
        }

        function exportDatabase() {
            window.location.href = '?dbaction=export&token=<?php echo isset($_SESSION['token']) ? $_SESSION['token'] : '' ?>';
        }

        function showSqlExecutor() {
            $('#db-result').show();
            $('#db-content').html(`
                <h5><?php echo lng('SQL Query Executor') ?></h5>
                <div class="mb-3">
                    <label for="sql-query" class="form-label"><?php echo lng('Enter SQL Query') ?>:</label>
                    <textarea class="form-control" id="sql-query" rows="6" placeholder="SELECT * FROM table_name LIMIT 10;"></textarea>
                    <div class="form-text"><?php echo lng('Enter your SQL query above. Be careful with UPDATE, DELETE, and DROP statements.') ?></div>
                </div>
                <div class="mb-3">
                    <button type="button" class="btn btn-primary" onclick="executeSql()">
                        <i class="fa fa-play"></i> <?php echo lng('Execute Query') ?>
                    </button>
                    <button type="button" class="btn btn-outline-secondary" onclick="clearSql()">
                        <i class="fa fa-eraser"></i> <?php echo lng('Clear') ?>
                    </button>
                </div>
        ...
        function executeSql() {
            ...
            if (!query) {
                alert('<?php echo lng('Please enter a SQL query') ?>');
                return;
            }

            $('#sql-result').html('<div class="text-center"><i class="fa fa-spinner fa-spin"></i> <?php echo lng('Executing query') ?>...</div>');
            ...
            $.ajax({
                ...
                error: function() {
                    $('#sql-result').html('<div class="alert alert-danger"><?php echo lng('Query execution failed') ?></div>');
                }
            });
        }
        ...
        // Add browse and structure functions for main database manager page
        function browseTable(tableName, offset) {
            browseTableWithSort(tableName, offset || 0);
        }

        function browseTableWithSort(tableName, offset, orderBy, orderDir) {
            offset = offset || 0;
            orderBy = orderBy || '';
            orderDir = orderDir || 'DESC';

            $('#db-result').show();
            $('#db-content').html('<div class="text-center"><i class="fa fa-spinner fa-spin"></i> <?php echo lng('Loading') ?>...</div>');

            var url = '?dbaction=browse&table=' + encodeURIComponent(tableName) + '&offset=' + offset;
            if (orderBy) {
                url += '&order_by=' + encodeURIComponent(orderBy) + '&order_dir=' + encodeURIComponent(orderDir);
            }
            ...
            $.ajax({
                ...
                data: { token: '<?php echo isset($_SESSION['token']) ? $_SESSION['token'] : '' ?>' },
                success: function(response) {
                    $('#db-content').html('<h5><?php echo lng('Browse Table') ?>: ' + tableName + '</h5>' + response);
                },
                error: function(xhr, status, error) {
                    $('#db-content').html('<div class="alert alert-danger"><?php echo lng('Error loading table data') ?>: ' + error + '</div>');
                }
            });
        }
        ...
        function showTableStructure(tableName) {
            ...
            $.ajax({
                url: '?dbaction=structure&table=' + encodeURIComponent(tableName),
                method: 'POST',
                data: { token: '<?php echo isset($_SESSION['token']) ? $_SESSION['token'] : '' ?>' },
                success: function(response) {
                    $('#db-content').html(response);
                ...
                error: function(xhr, status, error) {
                    $('#db-content').html('<div class="alert alert-danger"><?php echo lng('Error loading table structure') ?>: ' + error + '</div>');
                }
            });
        }
        </script>
    ...
    /**
     * Detect database environment and return configuration
     * @return array|false
     */
    function fm_detect_db_environment()
    {
        $root_path = FM_ROOT_PATH;

        // Check for OpenCart
        $opencart_config = $root_path . '/config.php';
        if (file_exists($opencart_config)) {
            return fm_parse_opencart_config($opencart_config);
        }

        // Check for WordPress
        $wp_config = $root_path . '/wp-config.php';
        if (file_exists($wp_config)) {
            return fm_parse_wordpress_config($wp_config);
        }
    ...
    /**
     * Parse OpenCart config.php
     * @param string $config_file
     * @return array
     */
    function fm_parse_opencart_config($config_file)
    {
        $content = file_get_contents($config_file);
        $config = array();

        // Extract database configuration
        if (preg_match("/define\s*\(\s*['\"]DB_DRIVER['\"]\s*,\s*['\"]([^'\"]*)['\"].*?\)/", $content, $matches)) {
            $config['driver'] = $matches[1];
        }
        if (preg_match("/define\s*\(\s*['\"]DB_HOSTNAME['\"]\s*,\s*['\"]([^'\"]*)['\"].*?\)/", $content, $matches)) {
        ...
        if (preg_match("/define\s*\(\s*['\"]DB_DATABASE['\"]\s*,\s*['\"]([^'\"]*)['\"].*?\)/", $content, $matches)) {
            $config['database'] = $matches[1];
        }
        if (preg_match("/define\s*\(\s*['\"]DB_PORT['\"]\s*,\s*['\"]([^'\"]*)['\"].*?\)/", $content, $matches)) {
            $config['port'] = $matches[1];
        }
        if (preg_match("/define\s*\(\s*['\"]DB_PREFIX['\"]\s*,\s*['\"]([^'\"]*)['\"].*?\)/", $content, $matches)) {
            $config['prefix'] = $matches[1];
        }

        $config['type'] = 'OpenCart';
        return $config;
    }
     ...
     * @return array
     */
    function fm_parse_wordpress_config($config_file)
    {
        $content = file_get_contents($config_file);
        $config = array();
     ...
     * @param string $action
     */
    function fm_handle_db_action($action)
    {
        // Verify token for security (except for phpmyadmin which is just a display page)
        if ($action !== 'phpmyadmin' && !isset($_POST['token']) && !isset($_GET['token'])) {
        ...
        $db_config = fm_detect_db_environment();
        if (!$db_config) {
            echo json_encode(['error' => 'No database configuration found']);
            return;
        }

        switch ($action) {
            case 'test':
                fm_test_db_connection($db_config);
                break;
            case 'tables':
                fm_show_db_tables($db_config);
                break;
            case 'browse':
                if (isset($_GET['table'])) {
                    fm_browse_table($db_config, $_GET['table']);
                } else {
            ...
            case 'structure':
                if (isset($_GET['table'])) {
                    fm_show_table_structure($db_config, $_GET['table']);
                } else {
                    echo json_encode(['error' => 'Table name required']);
                }
                break;
            case 'phpmyadmin':
                fm_show_phpmyadmin_style($db_config);
                break;
            case 'export':
                fm_export_database($db_config);
                break;
            case 'sql':
                fm_execute_sql($db_config);
                break;
            default:
     ...
     * @param array $config
     */
    function fm_test_db_connection($config)
    {
        try {
            ...
            echo '<strong>' . lng('Connection successful') . '</strong><br>';
            echo lng('MySQL Version') . ': ' . htmlspecialchars($result['version']);
            echo '</div>';
        } catch (PDOException $e) {
            echo '<div class="alert alert-danger">';
            echo '<strong>' . lng('Connection failed') . '</strong><br>';
     ...
     * @param array $config
     */
    function fm_show_db_tables($config)
    {
        try {
            $dsn = "mysql:host={$config['host']};port={$config['port']};dbname={$config['database']}";
            $pdo = new PDO($dsn, $config['username'], $config['password']);
            ...
            echo '<thead><tr>';
            echo '<th>' . lng('Table Name') . '</th>';
            echo '<th>' . lng('Engine') . '</th>';
            echo '<th>' . lng('Rows') . '</th>';
            echo '<th>' . lng('Data Size') . '</th>';
            echo '<th>' . lng('Index Size') . '</th>';
            echo '<th>' . lng('Actions') . '</th>';
            echo '</tr></thead><tbody>';

            foreach ($tables as $table) {
                $data_size = fm_get_filesize($table['Data_length']);
                $index_size = fm_get_filesize($table['Index_length']);

                echo '<tr>';
                echo '<td><strong>' . htmlspecialchars($table['Name']) . '</strong></td>';
                echo '<td>' . htmlspecialchars($table['Engine']) . '</td>';
                echo '<td>' . number_format($table['Rows']) . '</td>';
                echo '<td>' . $data_size . '</td>';
                echo '<td>' . $index_size . '</td>';
                echo '<td>';
                echo '<button class="btn btn-sm btn-outline-primary" onclick="browseTable(\'' . htmlspecialchars($table['Name']) . '\'); return false;">' . lng('Browse') . '</button> ';
                echo '<button class="btn btn-sm btn-outline-info" onclick="showTableStructure(\'' . htmlspecialchars($table['Name']) . '\'); return false;">' . lng('Structure') . '</button>';
                echo '</td>';
                echo '</tr>';
            }
            ...
            echo '<div class="alert alert-danger">';
            echo '<strong>' . lng('Error') . '</strong><br>';
            echo htmlspecialchars($e->getMessage());
            echo '</div>';
        }
    }
     ...
     * @param string $table
     */
    function fm_browse_table($config, $table)
    {
        try {
            $dsn = "mysql:host={$config['host']};port={$config['port']};dbname={$config['database']}";
            $pdo = new PDO($dsn, $config['username'], $config['password']);
            ...
            // Get table data with limit and sorting
            ...
            if (!empty($order_by)) {
                ...
                $columns_stmt = $pdo->prepare("SHOW COLUMNS FROM `{$table}`");
                $columns_stmt->execute();
                $valid_columns = $columns_stmt->fetchAll(PDO::FETCH_COLUMN);
            ...
            // Get total count
            $count_stmt = $pdo->prepare("SELECT COUNT(*) as total FROM `{$table}`");
            $count_stmt->execute();
            $total = $count_stmt->fetch(PDO::FETCH_ASSOC)['total'];

            if (empty($rows)) {
                echo '<div class="alert alert-info">' . lng('No data found in table') . '</div>';
                return;
            }

            echo '<div class="d-flex justify-content-between mb-3">';
            echo '<h6>' . lng('Table') . ': ' . htmlspecialchars($table) . ' (' . number_format($total) . ' ' . lng('rows') . ')</h6>';
            echo '<div>';
            if ($offset > 0) {
                $prev_offset = max(0, $offset - $limit);
                $sort_params = !empty($order_by) ? ', \'' . htmlspecialchars($order_by) . '\', \'' . $order_dir . '\'' : '';
                echo '<button class="btn btn-sm btn-outline-primary me-2" onclick="browseTableWithSort(\'' . htmlspecialchars($table) . '\', ' . $prev_offset . $sort_params . ')">' . lng('Previous') . '</button>';
            }
            if ($offset + $limit < $total) {
                $next_offset = $offset + $limit;
                $sort_params = !empty($order_by) ? ', \'' . htmlspecialchars($order_by) . '\', \'' . $order_dir . '\'' : '';
                echo '<button class="btn btn-sm btn-outline-primary" onclick="browseTableWithSort(\'' . htmlspecialchars($table) . '\', ' . $next_offset . $sort_params . ')">' . lng('Next') . '</button>';
            }
            echo '</div>';
            echo '</div>';
            ...
            foreach ($columns as $column) {
                ...
                if ($order_by === $column) {
                    if ($order_dir === 'DESC') {
                        $sort_icon = ' <i class="fa fa-sort-desc"></i>';
                        $next_dir = 'ASC';
                    } else {
                        $sort_icon = ' <i class="fa fa-sort-asc"></i>';
                        $next_dir = 'DESC';
                    }
                } else {
                ...
                echo '<th>';
                echo '<a href="javascript:void(0)" onclick="browseTableWithSort(\'' . htmlspecialchars($table) . '\', 0, \'' . htmlspecialchars($column) . '\', \'' . $next_dir . '\')" class="text-decoration-none">';
                echo htmlspecialchars($column) . $sort_icon;
                echo '</a>';
                echo '</th>';
            }
            echo '</tr></thead><tbody>';
     ...
     * @param string $table
     */
    function fm_show_table_structure($config, $table)
    {
        try {
            $dsn = "mysql:host={$config['host']};port={$config['port']};dbname={$config['database']}";
            $pdo = new PDO($dsn, $config['username'], $config['password']);
            ...
            echo '<h6>' . lng('Table Structure') . ': ' . htmlspecialchars($table) . '</h6>';
            echo '<div class="table-responsive">';
            echo '<table class="table table-striped table-sm">';
            echo '<thead><tr>';
            echo '<th>' . lng('Field') . '</th>';
            echo '<th>' . lng('Type') . '</th>';
            echo '<th>' . lng('Null') . '</th>';
            echo '<th>' . lng('Key') . '</th>';
            echo '<th>' . lng('Default') . '</th>';
            echo '<th>' . lng('Extra') . '</th>';
            echo '</tr></thead><tbody>';
    ...
    /**
     * Show phpMyAdmin style interface
     * @param array $config
     */
    function fm_show_phpmyadmin_style($config)
    {
        fm_show_header();
        ?>
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-3">
                    <div class="card">
                        <div class="card-header">
                            <h6><i class="fa fa-database"></i> <?php echo htmlspecialchars($config['database']) ?></h6>
                        </div>
                        <div class="card-body" id="table-list">
                            <div class="text-center">
                                <i class="fa fa-spinner fa-spin"></i> <?php echo lng('Loading') ?>...
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-md-9">
                    <div class="card">
                        <div class="card-header">
                            <h6><i class="fa fa-table"></i> <span id="current-table"><?php echo lng('Select a table') ?></span></h6>
                        </div>
                        <div class="card-body" id="table-content">
                            <div class="text-center text-muted">
                                <?php echo lng('Select a table from the left panel') ?>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        function loadTableList() {
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    $('#table-list').html('<div class="alert alert-danger">Error loading tables: ' + error + '</div>');
        ...
        function browseTableWithSort(tableName, offset, orderBy, orderDir) {
            ...
            $('#table-content').html('<div class="text-center"><i class="fa fa-spinner fa-spin"></i> <?php echo lng('Loading') ?>...</div>');
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    $('#table-content').html('<div class="alert alert-danger">Error: ' + error + '</div>');
                }
            });
        }

        function showTableStructure(tableName) {
            $('#current-table').text(tableName + ' - Structure');
            $('#table-content').html('<div class="text-center"><i class="fa fa-spinner fa-spin"></i> <?php echo lng('Loading') ?>...</div>');
     ...
     * @param array $config
     */
    function fm_export_database($config)
    {
        try {
            $dsn = "mysql:host={$config['host']};port={$config['port']};dbname={$config['database']}";
            $pdo = new PDO($dsn, $config['username'], $config['password']);
            ...
            $filename = $config['database'] . '_' . date('Y-m-d_H-i-s') . '.sql';
            ...
            echo "-- Database export for {$config['database']}\n";
     ...
     * @param array $config
     */
    function fm_execute_sql($config)
    {
        if (!isset($_POST['sql']) || empty(trim($_POST['sql']))) {
            echo '<div class="alert alert-danger">' . lng('No SQL query provided') . '</div>';
            return;
        }
        ...
        try {
            ...
            if ($is_select) {
                ...
                if (empty($results)) {
                    echo '<div class="alert alert-info">' . lng('Query executed successfully. No results returned.') . '</div>';
                } else {
                    echo '<div class="alert alert-success">' . lng('Query executed successfully') . '. ' . count($results) . ' ' . lng('rows returned') . '.</div>';
                    ...
                    // Table data (limit to first 100 rows for performance)
                    $display_results = array_slice($results, 0, 100);
                    ...
                    if (count($results) > 100) {
                        echo '<div class="alert alert-info">Only first 100 rows displayed. Total: ' . count($results) . ' rows.</div>';
                    }
                }
            } else {
                ...
                echo '<div class="alert alert-success">';
                echo lng('Query executed successfully') . '. ';
                echo $affected_rows . ' ' . lng('rows affected') . '.';
                echo '</div>';
            }

        } catch (PDOException $e) {
            echo '<div class="alert alert-danger">';
            echo '<strong>' . lng('SQL Error') . '</strong><br>';
            echo htmlspecialchars($e->getMessage());
            echo '</div>';
        }
    }
    ...
    /**
     * Show PHP code executor page
     */
    function fm_show_php_executor()
    {
        fm_show_header();
        ?>
        <div class="path">
            <div class="card mb-2" data-bs-theme="<?php echo FM_THEME; ?>">
                <h6 class="card-header d-flex justify-content-between">
                    <span><i class="fa fa-code"></i> <?php echo lng('PHP Code Executor') ?></span>
                    <a href="?p=<?php echo urlencode(FM_PATH) ?>" class="text-danger"><i class="fa fa-times-circle-o"></i> <?php echo lng('Back') ?></a>
                </h6>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="alert alert-warning">
                                <strong><i class="fa fa-exclamation-triangle"></i> <?php echo lng('Security Warning') ?>:</strong><br>
                                <?php echo lng('This tool allows execution of arbitrary PHP code. Use with extreme caution!') ?><br>
                                <?php echo lng('Only execute code from trusted sources. Malicious code can damage your system.') ?>
                            </div>

                            <div class="mb-3">
                                <label for="php-code" class="form-label"><strong><?php echo lng('PHP Code') ?>:</strong></label>
                                <textarea class="form-control" id="php-code" name="php-code" rows="15" placeholder="<?php echo lng('Enter your PHP code here...') ?>"><?php echo htmlspecialchars('<?php
// Example: Display current date and time
echo "Current date and time: " . date("Y-m-d H:i:s") . "\n";

// Example: Show PHP version
...
?>'); ?></textarea>
                                <div class="form-text">
                                    <?php echo lng('Enter PHP code above. You can use opening and closing PHP tags or just plain PHP code.') ?>
                                </div>
                            </div>

                            <div class="mb-3">
                                <div class="btn-group">
                                    <button type="button" class="btn btn-success" onclick="executePhpCode()">
                                        <i class="fa fa-play"></i> <?php echo lng('Execute Code') ?>
                                    </button>
                                    <button type="button" class="btn btn-outline-secondary" onclick="clearPhpCode()">
                                        <i class="fa fa-eraser"></i> <?php echo lng('Clear') ?>
                                    </button>
                                    <button type="button" class="btn btn-outline-info" onclick="loadExample()">
                                        <i class="fa fa-lightbulb-o"></i> <?php echo lng('Load Example') ?>
                                    </button>
                                    <button type="button" class="btn btn-outline-warning" onclick="showPhpInfo()">
                                        <i class="fa fa-info-circle"></i> <?php echo lng('PHP Info') ?>
                                    </button>
                                </div>
                            </div>

                            <div id="php-result" class="mt-4" style="display: none;">
                                <h5><?php echo lng('Execution Result') ?></h5>
        ...
        function executePhpCode() {
            var code = $('#php-code').val().trim();
            if (!code) {
                alert('<?php echo lng('Please enter PHP code to execute') ?>');
                return;
            }

            if (!confirm('<?php echo lng('Are you sure you want to execute this PHP code? This action cannot be undone.') ?>')) {
                return;
            }

            $('#php-result').show();
            $('#php-output').html('<div class="text-center"><i class="fa fa-spinner fa-spin"></i> <?php echo lng('Executing code') ?>...</div>');

            $.ajax({
                url: '?phpaction=execute',
                method: 'POST',
                data: {
                    token: '<?php echo isset($_SESSION['token']) ? $_SESSION['token'] : '' ?>',
                    code: code
                ...
                success: function(response) {
                    $('#php-output').html(response);
                },
                error: function(xhr, status, error) {
                    $('#php-output').html('<div class="alert alert-danger"><?php echo lng('Code execution failed') ?>: ' + error + '</div>');
                }
            });
        }

        function clearPhpCode() {
            $('#php-code').val('');
            $('#php-result').hide();
            $('#php-output').html('');
        }
...
$files = array_slice(scandir('.'), 0, 10); // Show first 10 files
foreach($files as $file) {
    if($file != '.' && $file != '..') {
        echo "- " . $file . "\\n";
    }
...
?>`;
            $('#php-code').val(example);
        }

        function showPhpInfo() {
            $('#php-result').show();
            $('#php-output').html('<div class="text-center"><i class="fa fa-spinner fa-spin"></i> <?php echo lng('Loading PHP Info') ?>...</div>');

            $.ajax({
                url: '?phpaction=phpinfo',
                method: 'POST',
                data: {
                    token: '<?php echo isset($_SESSION['token']) ? $_SESSION['token'] : '' ?>'
                },
                ...
                error: function(xhr, status, error) {
                    $('#php-output').html('<div class="alert alert-danger"><?php echo lng('Failed to load PHP Info') ?>: ' + error + '</div>');
                }
            });
        }
        </script>
    ...
    /**
     * Handle PHP code actions
     * @param string $action
     */
    function fm_handle_php_action($action)
    {
     ...
     * Execute PHP code
     */
    function fm_execute_php_code()
    {
        if (!isset($_POST['code']) || empty(trim($_POST['code']))) {
            echo '<div class="alert alert-danger">' . lng('No PHP code provided') . '</div>';
            return;
        }
        ...
        set_error_handler(function($severity, $message, $file, $line) use (&$error_occurred, &$error_message) {
            $error_occurred = true;
            $error_message = "Error: $message in line $line";
        });
        ...
        if ($error_occurred) {
            echo '<div class="alert alert-danger">';
            echo '<strong>' . lng('Execution Error') . '</strong><br>';
            echo '<pre>' . htmlspecialchars($error_message) . '</pre>';
            echo '</div>';
        } else {
            echo '<div class="alert alert-success">';
            echo '<strong>' . lng('Code executed successfully') . '</strong>';
            echo '</div>';

            if (!empty($output)) {
                echo '<div class="card">';
                echo '<div class="card-header"><strong>' . lng('Output') . '</strong></div>';
                echo '<div class="card-body">';
                echo '<pre class="mb-0">' . htmlspecialchars($output) . '</pre>';
                echo '</div>';
                echo '</div>';
            } else {
                echo '<div class="alert alert-info">' . lng('Code executed successfully but produced no output') . '</div>';
            }
        }
    }
     ...
     * Show PHP Info
     */
    function fm_show_phpinfo()
    {
        ob_start();
        phpinfo();
        $phpinfo = ob_get_clean();

        // Extract body content and apply custom styling
        if (preg_match('/<body[^>]*>(.*?)<\/body>/is', $phpinfo, $matches)) {
            $content = $matches[1];

            // Apply Bootstrap classes to tables
            $content = str_replace('<table', '<table class="table table-striped table-sm"', $content);
            ...
            echo '<div class="card">';
            echo '<div class="card-header"><strong>' . lng('PHP Information') . '</strong></div>';
            echo '<div class="card-body">';
            ...
            echo '<div class="alert alert-danger">' . lng('Failed to generate PHP Info') . '</div>';
        }
    }
    ...
    function lng($txt)
    {
        ...
        $tr['en']['Last Modified Timestamp']                        = 'Last Modified Timestamp';
        $tr['en']['Change Modification Time']                       = 'Change Modification Time';
        $tr['en']['Change Time']                                    = 'Change Time';
        $tr['en']['Current modification time']                      = 'Current modification time';
        $tr['en']['New modification time']                          = 'New modification time';
        $tr['en']['Select new modification date and time']          = 'Select new modification date and time';
        $tr['en']['Set to current time']                            = 'Set to current time';
        $tr['en']['Reset to original']                              = 'Reset to original';
        $tr['en']['Current timestamp']                              = 'Current timestamp';
        $tr['en']['Supported formats']                              = 'Supported formats';
        $tr['en']['Please use supported formats']                   = 'Please use supported formats';
        $tr['en']['File modification time changed']                 = 'File modification time changed';
        $tr['en']['File modification time not changed']             = 'File modification time not changed';
        $tr['en']['Invalid date format']                            = 'Invalid date format';
        $tr['en']['Database Manager']                               = 'Database Manager';
        $tr['en']['Environment Detected']                           = 'Environment Detected';
        $tr['en']['Database Configuration']                         = 'Database Configuration';
        $tr['en']['Driver']                                         = 'Driver';
        $tr['en']['Host']                                           = 'Host';
        $tr['en']['Port']                                           = 'Port';
        $tr['en']['Database']                                       = 'Database';
        $tr['en']['Username']                                       = 'Username';
        $tr['en']['Password']                                       = 'Password';
        $tr['en']['Table Prefix']                                   = 'Table Prefix';
        $tr['en']['Database Actions']                               = 'Database Actions';
        $tr['en']['Test Connection']                                = 'Test Connection';
        $tr['en']['Show Tables']                                    = 'Show Tables';
        $tr['en']['Open phpMyAdmin Style']                          = 'Open phpMyAdmin Style';
        $tr['en']['Export Database']                                = 'Export Database';
        $tr['en']['Result']                                         = 'Result';
        $tr['en']['No Database Configuration Found']                = 'No Database Configuration Found';
        $tr['en']['Supported environments']                         = 'Supported environments';
        $tr['en']['Testing connection']                             = 'Testing connection';
        $tr['en']['Connection failed']                              = 'Connection failed';
        $tr['en']['Loading tables']                                 = 'Loading tables';
        $tr['en']['Failed to load tables']                          = 'Failed to load tables';
        $tr['en']['Connection successful']                          = 'Connection successful';
        $tr['en']['MySQL Version']                                  = 'MySQL Version';
        $tr['en']['Table Name']                                     = 'Table Name';
        $tr['en']['Engine']                                         = 'Engine';
        $tr['en']['Rows']                                           = 'Rows';
        $tr['en']['Data Size']                                      = 'Data Size';
        $tr['en']['Index Size']                                     = 'Index Size';
        $tr['en']['Actions']                                        = 'Actions';
        $tr['en']['Browse']                                         = 'Browse';
        $tr['en']['Structure']                                      = 'Structure';
        $tr['en']['Error']                                          = 'Error';
        $tr['en']['Loading']                                        = 'Loading';
        $tr['en']['Select a table']                                 = 'Select a table';
        $tr['en']['Select a table from the left panel']             = 'Select a table from the left panel';
        $tr['en']['Access Denied']                                  = 'Access Denied';
        $tr['en']['Database management is not available in read-only mode'] = 'Database management is not available in read-only mode';
        $tr['en']['Please login with an administrator account to access this feature'] = 'Please login with an administrator account to access this feature';
        $tr['en']['Execute SQL']                                    = 'Execute SQL';
        $tr['en']['SQL Query Executor']                             = 'SQL Query Executor';
        $tr['en']['Enter SQL Query']                                = 'Enter SQL Query';
        $tr['en']['Enter your SQL query above. Be careful with UPDATE, DELETE, and DROP statements.'] = 'Enter your SQL query above. Be careful with UPDATE, DELETE, and DROP statements.';
        $tr['en']['Execute Query']                                  = 'Execute Query';
        $tr['en']['Clear']                                          = 'Clear';
        $tr['en']['Please enter a SQL query']                      = 'Please enter a SQL query';
        $tr['en']['Executing query']                                = 'Executing query';
        $tr['en']['Query execution failed']                         = 'Query execution failed';
        $tr['en']['No SQL query provided']                          = 'No SQL query provided';
        $tr['en']['Query executed successfully. No results returned.'] = 'Query executed successfully. No results returned.';
        $tr['en']['Query executed successfully']                    = 'Query executed successfully';
        $tr['en']['rows returned']                                  = 'rows returned';
        $tr['en']['rows affected']                                  = 'rows affected';
        $tr['en']['SQL Error']                                      = 'SQL Error';
        $tr['en']['No data found in table']                         = 'No data found in table';
        $tr['en']['Table']                                          = 'Table';
        $tr['en']['rows']                                           = 'rows';
        $tr['en']['Previous']                                       = 'Previous';
        $tr['en']['Next']                                           = 'Next';
        $tr['en']['Table Structure']                                = 'Table Structure';
        $tr['en']['Field']                                          = 'Field';
        $tr['en']['Type']                                           = 'Type';
        $tr['en']['Null']                                           = 'Null';
        $tr['en']['Key']                                            = 'Key';
        $tr['en']['Default']                                        = 'Default';
        $tr['en']['Extra']                                          = 'Extra';
        $tr['en']['Browse Table']                                   = 'Browse Table';
        $tr['en']['Error loading table data']                       = 'Error loading table data';
        $tr['en']['Error loading table structure']                  = 'Error loading table structure';
        $tr['en']['PHP Code Executor']                              = 'PHP Code Executor';
        $tr['en']['PHP Executor']                                   = 'PHP Executor';
        $tr['en']['PHP code execution is not available in read-only mode'] = 'PHP code execution is not available in read-only mode';
        $tr['en']['Security Warning']                               = 'Security Warning';
        $tr['en']['This tool allows execution of arbitrary PHP code. Use with extreme caution!'] = 'This tool allows execution of arbitrary PHP code. Use with extreme caution!';
        $tr['en']['Only execute code from trusted sources. Malicious code can damage your system.'] = 'Only execute code from trusted sources. Malicious code can damage your system.';
        $tr['en']['PHP Code']                                       = 'PHP Code';
        $tr['en']['Enter your PHP code here...']                    = 'Enter your PHP code here...';
        $tr['en']['Enter PHP code above. You can use opening and closing PHP tags or just plain PHP code.'] = 'Enter PHP code above. You can use opening and closing PHP tags or just plain PHP code.';
        $tr['en']['Execute Code']                                   = 'Execute Code';
        $tr['en']['Load Example']                                   = 'Load Example';
        $tr['en']['PHP Info']                                       = 'PHP Info';
        $tr['en']['Execution Result']                               = 'Execution Result';
        $tr['en']['Please enter PHP code to execute']              = 'Please enter PHP code to execute';
        $tr['en']['Are you sure you want to execute this PHP code? This action cannot be undone.'] = 'Are you sure you want to execute this PHP code? This action cannot be undone.';
        $tr['en']['Code execution failed']                          = 'Code execution failed';
        $tr['en']['Loading PHP Info']                               = 'Loading PHP Info';
        $tr['en']['Failed to load PHP Info']                       = 'Failed to load PHP Info';
        $tr['en']['No PHP code provided']                           = 'No PHP code provided';
        $tr['en']['Security Error']                                 = 'Security Error';
        $tr['en']['The function']                                   = 'The function';
        $tr['en']['is not allowed for security reasons']            = 'is not allowed for security reasons';
        $tr['en']['Execution Error']                                = 'Execution Error';
        $tr['en']['Code executed successfully']                     = 'Code executed successfully';
        $tr['en']['Output']                                         = 'Output';
        $tr['en']['Code executed successfully but produced no output'] = 'Code executed successfully but produced no output';
        $tr['en']['PHP Information']                                = 'PHP Information';
        $tr['en']['Failed to generate PHP Info']                   = 'Failed to generate PHP Info';
